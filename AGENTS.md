...
- **Pas de logique dans les `model`** — les records restent de simples porteurs de données ; toute logique impérative (formatage, calcul, règles métier) vit dans le `service`. Les annotations déclaratives de Bean Validation (`@NotBlank`, `@Size`, etc.) sur les champs d'un record DTO sont acceptables : ce sont des métadonnées de contrat, pas de la logique.
